...
const socket = net.createConnection({ port: PORT, host: HOST }, async () => {
	...
	console.log(); // Avoid clearing the command line for the %
	...
	// When the internal buffer of the socket is drained when the OS does it (thanks to libuv) it's safe to resume
	socket.on("drain", () => {
		fileStream.resume();
	});
